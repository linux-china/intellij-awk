...
BUILTIN_FUNC_NAME_ZAWK=match|hex|join_fields|join_csv|join_tsv|strtonum|trim|truncate|capitalize|uncapitalize|camel_case|kebab_case|snake_case|title_case|isint|isnum|mask|pad|pad_start|pad_end|strcmp|words|repeat|default_if_empty | append_if_missing | prepend_if_missing | double_quote | quote |escape|escape_csv|escape_tsv|url|data_url|shlex|path|semver|pairs|attributes|message|func|uuid|ulid|snowflake|seq|uniq|asort|_max|_min|_sum|_mean|_join|abs|floor|ceil|round|min|max|fend|mkbool|int|float|to_json|from_json|to_csv|from_csv|encode|decode|digest|hmac|jwt|kv_get|kv_put|kv_delete|kv_clear|local_ip|http_get|http_post|s3_get|s3_put|publish|sqlite_query|sqlite_execute|mysql_query|mysql_execute|datetime|whoami|os|arch|os_family|pwd|user_home|read_all|write_all|var_dump|log_debug|log_info|log_warn|log_error
