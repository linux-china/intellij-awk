...
BUILTIN_FUNC_NAME_ZAWK=fend|abs|floor|ceil|round|min|max|trim|truncate|escape|capitalize|seq|_join|local_ip|http_get|http_post|s3_get|s3_put|url|publish|whoami|encode|decode|digest|hmac|to_json|from_json|kv_get|kv_put|kv_delete|kv_clear|uuid|ulid|snowflake|datetime
